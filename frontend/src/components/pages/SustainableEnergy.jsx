...
const SustainableEnergy = () => {
  ...
  const calculateProjectsFromEvents = (eventsData) => {
    const sustainableEnergyEvents = eventsData.filter(event => 
      event.pillar && event.pillar.includes('Sustainable Energy')
    );
    
    // Group events by office to create projects
    const officeGroups = sustainableEnergyEvents.reduce((acc, event) => {
    ...
    const loadEvents = async () => {
      try {
        ...
        setEvents([
          {
            id: 1,
            event_name: 'Solar Street Lighting',
            date: '2026-03-15',
            duration: '3 days',
            pillar: '6. Sustainable Energy',
  ...
  const stats = projects.length > 0 ? {
    ...
    totalProjects: events.filter(event => event.pillar && event.pillar.includes('Sustainable Energy')).length
  } : {
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Sustainable Energy</h1>
        <p className="text-gray-600">Programs promoting renewable energy and energy efficiency initiatives</p>
      </div>

      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg shadow-md p-6 border border-yellow-300">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-yellow-700 font-medium">Leading Office</p><p className="text-xl font-bold text-yellow-900 mt-1">{stats.leadingOffice}</p></div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-lg shadow-md p-6 border border-amber-300">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-amber-700 font-medium">Overall Accomplishment</p><p className="text-2xl font-bold text-amber-900">{stats.overallAccomplishment}%</p></div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg shadow-md p-6 border border-orange-300">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-orange-700 font-medium">Total Departments</p><p className="text-2xl font-bold text-orange-900">{stats.totalDepartments}</p></div>
        ...
        <div className="bg-gradient-to-br from-lime-50 to-lime-100 rounded-lg shadow-md p-6 border border-lime-300">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-lime-700 font-medium">Total Projects</p><p className="text-2xl font-bold text-lime-900">{stats.totalProjects}</p></div>
            <div className="bg-lime-600 text-white p-3 rounded-lg">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={handleAdd} className="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors flex items-center gap-2">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-1 bg-gray-200 rounded-full h-2 mr-3">
                        <div className="bg-yellow-600 h-2 rounded-full" style={{ width: `${project.accomplishment}%` }}></div>
                      </div>
      ...
      {/* Modals (simplified - same structure as other pages) */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 m-4 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Office</label><select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"><option>City Engineering Office</option><option>City Energy Office</option></select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Project Name</label><input type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Accomplishment (%)</label><input type="number" min="0" max="100" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Status</label><select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"><option>Ongoing</option><option>Completed</option><option>Pending</option></select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Target</label><input type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Actual</label><input type="text" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Quarter</label><select className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"><option>Q1 2026</option><option>Q2 2026</option><option>Q3 2026</option><option>Q4 2026</option></select></div>
              </div>
              <div className="flex justify-end gap-2 mt-6">
                ...
                <button type="submit" className="px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors">{isEditModalOpen ? 'Update' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
